...
module.exports = class AssetPipeClient {
    ...
    publish(options = { js: null, css: null }) {
        ow(options, ow.object.label('options').hasAnyKeys('js', 'css'));
        const { js = null, css = null } = options;
        ow(
            js,
            ow.any(ow.null.label('options.js'), ow.string.label('options.js'))
        );
        ow(
            css,
            ow.any(ow.null.label('options.css'), ow.string.label('options.css'))
        );
    ...
    bundle(options = { js: [], css: [] }) {
        ...
        ow(js, ow.array.label('options.js').ofType(ow.string));
        ow(css, ow.array.label('options.css').ofType(ow.string));
    ...
    assetUrlByType(hashes = [], type) {
        ow(hashes, ow.array.label('hashes').ofType(ow.string));
        ow(type, ow.string.label('type').oneOf(['js', 'css']));

        const tags = this.instructions[type];
        if (!tags.length) {
            this.log.trace(
                `${type} "tags" is an empty array, did you forget to call .bundle(options) before calling .scripts() or .styles()?`
            );
            return [];
        }
        if (!hashes.length) {
        ...
        const metricEnd = this.metrics.timer({
            name: 'calculate_asset_bundle_urls_timer',
            description: 'Time spent calculating asset bundle URLs',
            meta: { type },
        });
        ...
        if (tags.length === hashes.length) {
            ...
            if (cachedBundleURL === calculatedBundleURL) {
                ...
                metricEnd({ meta: { status: 'in cache' } });
            ...
            metricEnd({ meta: { status: 'not ready' } });
        }
    ...
    async ready() {
        if (this.development) {
            this.log.trace(
                `client.ready(): "development" mode set to "true", immediately resolving with "true"`
            );
            return Promise.resolve(true);
        }

        const metricEnd = this.metrics.timer({
            name: 'publish_and_bundle_readiness_timer',
            description:
                'Time spent waiting for publishing and bundling to complete after calling ready',
        });
...
// TODO: add metrics to scripts

// TODO: add metrics to styles

// TODO: add metrics to ready
