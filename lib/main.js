...
module.exports = class Client {
    ...
    async publishJsAssets(tag, files, options) {
        const writer = new JsWriter(files, options);
        ...
        return post({
            ...
            body: JSON.stringify({ tag, type: 'js', data }),
            serverId: this.serverId,
        });
    }

    async publishCssAssets(tag, files) {
        const writer = new CssWriter(files);
        const data = await getStream(writer.bundle());
        ...
        return post({
            ...
            body: JSON.stringify({ tag, type: 'css', data }),
            serverId: this.serverId,
        });
    }

    async publishAssets(tag, files, options) {
        assert(
            tag && typeof tag === 'string',
            `Expected 'tag' to be a string, instead got ${tag}`
        );
        assert(
            Array.isArray(files),
            `Expected 'files' to be an array, instead got ${files}`
        ...
        switch (extname(files[0])) {
            case extensions.CSS:
                upload = await this.publishCssAssets(tag, files);
                break;
            case extensions.JS:
                upload = await this.publishJsAssets(tag, files, options);
                break;
        }
    ...
    async publishInstructions(tag, type, data) {
        ...
        assert(
            Array.isArray(data),
            `Expected argument 'data' to be an array. Instead got ${typeof data}`
        );
        assert(
            data.every(source => typeof source === 'string'),
            `Expected all entries in array 'data' to be strings. Instead got ${data}`
        );
        assert(
            assetTypes.includes(type),
            `Expected argument 'type' to be one of ${assetTypes.join(
