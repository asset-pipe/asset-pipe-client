...
module.exports = class AssetPipeClient {
    constructor({
        ...
        this.readinessMetric = this.metrics.histogram({
        ...
        this.urlsMetric = this.metrics.histogram({
        ...
        this.syncMetric = this.metrics.histogram({
    ...
    async sync() {
        if (!this.publicFeedUrl || !this.publicBundleUrl) {
            const metricEnd = this.syncMetric.timer({
                labels: { statusCode: null }
            });
    ...
    assetUrlByType(hashes = [], type) {
        ...
        const metricEnd = this.urlsMetric.timer({
            labels: { type }
        });
    ...
    async ready() {
        const metricEnd = this.readinessMetric.timer({});
