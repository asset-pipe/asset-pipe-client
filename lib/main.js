...
module.exports = class AssetPipeClient {
    ...
    assetUrlByType(hashes, type) {
        ...
        if (this.instructions[type].length === hashes.length) {
            if (this.bundleURLs[type] === this.bundleURL(hashes, { type })) {
                return [this.bundleURLs[type]];
            }
            ...
            if (!this.verifyingBundle) {
                ...
                this.bundlingComplete(hashes)
                    .then(() => {
                        this.bundleURLs[type] = this.bundleURL(hashes, {
                            type,
                        });
